...
KIND_CONFIG="${REPO_ROOT}/platform/local/kind/cluster.yaml"
...
maybe_run_system_smoketest() {
  ...
  CLUSTER_NAME="${CLUSTER_NAME}" "${SCRIPT_DIR}/load-local-images-kind.sh"
...
main() {
  ...
  echo
  echo "Local CNPG/Postgres smoke path complete on kind cluster '${CLUSTER_NAME}'."
}
